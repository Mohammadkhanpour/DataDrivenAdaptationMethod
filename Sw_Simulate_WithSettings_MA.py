...
class SW_Sim_WithSettings_MA(object):
    ServiceMonitor = np.zeros((3, 3))
    Vec_fail = np.array([1.0, 1.0, 1.0])
    ...
    def callService(self,serviceNumber):
        ...
        assert execTime > 0.0
        failure = 1.0 if round(random.random(),2) > reliability_settings[serviceNumber] else 0.0
    ...
    def SW_Model2(self,Conf):
        ...
        requestsPerTimeUnit = 10000 #20000#100000
        ...
        for request in range(0, requestsPerTimeUnit - 3):
            dice = random.randrange(1, 10)/10 #random.random()
            if dice < self.p1:
                self.callService(0)
            else:
                dice = random.randrange(1, 10)/10 #random.random()
        ...
        #####################
        self.ServiceMonitor = [[self.p1 * self.Vec_fail[0] * measuredCost[0],
                                self.p1 * self.Vec_fail[0] * measuredReliability[0],
                                self.p1 * self.Vec_fail[0] * measuredExecTime[0]],
                               [(1.0 - self.p1) * (self.p2 * self.Vec_fail[1] * measuredCost[1]),
                                (1.0 - self.p1) * (self.p2 * self.Vec_fail[1] * measuredReliability[1]),
                                (1.0 - self.p1) * (self.p2 * self.Vec_fail[1] * measuredExecTime[1])],
                               [(1.0 - self.p1) * (1.0 - self.p2) * self.Vec_fail[2] * measuredCost[2],
                                (1.0 - self.p1) * (1.0 - self.p2) * self.Vec_fail[2] * measuredReliability[2],
                                (1.0 - self.p1) * (1.0 - self.p2) * self.Vec_fail[2] * measuredExecTime[2]]]

        # Composed estimate -> measure
        globalMeasuredReliability = self.p1 * self.Vec_fail[0] * measuredReliability[0] + (1.0 - self.p1) * (
                self.p2 * self.Vec_fail[1] * measuredReliability[1] + (1.0 - self.p2) * self.Vec_fail[2] *
                measuredReliability[2])
        globalMeasuredExecTime = self.p1 * self.Vec_fail[0] * measuredExecTime[0] + (1.0 - self.p1) * (
                self.p2 * self.Vec_fail[1] * measuredExecTime[1] + (1.0 - self.p2) * self.Vec_fail[2] *
                measuredExecTime[2])
        globalMeasuredCost = self.p1 * self.Vec_fail[0] * measuredCost[0] + (1.0 - self.p1) * (
                self.p2 * self.Vec_fail[1] * measuredCost[1] + (1.0 - self.p2) * self.Vec_fail[2] * measuredCost[2])
